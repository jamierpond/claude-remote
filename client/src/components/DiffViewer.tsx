...
export default function DiffViewer({ projectId, serverId, serverUrl, isOpen, onClose }: DiffViewerProps) {
  ...
  const fetchDiff = useCallback(async (filePath: string) => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/projects/${encodeURIComponent(projectId)}/diff/${encodedPath}`, { serverId, serverUrl });
      if (!res.ok) {
        const data = await res.json();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-md bg-[var(--color-bg-secondary)] rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[80vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-[var(--color-border-default)]">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Changes</h2>
          <button
            onClick={onClose}
            className="p-1 text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors"
            aria-label="Close"
          ...
          {loading ? (
            ...
            <div className="space-y-0.5">
              {files.map((file) => (
                ...
                >
                  {/* Status badge */}
                  <span className={`shrink-0 text-xs font-mono font-bold px-1.5 py-0.5 rounded ${statusColor(file.status)}`}>
