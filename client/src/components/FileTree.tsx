    ...
    async (subPath: string) => {
      ...
      try {
        ...
        const res = await apiFetch(
          `/api/projects/${encodeURIComponent(projectId)}/tree${pathSuffix}`,
          { serverId, serverUrl },
        );
    ...
    async (filePath: string) => {
      ...
      try {
        const encodedPath = filePath
          ...
          .join("/");
        const res = await apiFetch(
          `/api/projects/${encodeURIComponent(projectId)}/blob/${encodedPath}`,
          { serverId, serverUrl },
        );
        if (!res.ok) {
          const data = await res.json();
            ...
            <div className="space-y-0.5">
              {entries.map((entry) => {
                const fullPath = currentPath
                  ...
                  : entry.name;
                const fileChange =
                  entry.type === "file" ? changedFiles.get(fullPath) : null;
                const isDirDirty =
                  entry.type === "dir" && dirtyDirs.has(fullPath);
                  ...
                  >
                    ...
                    {entry.type === "dir" ? (
                      ...
                      <div className="relative shrink-0">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className={`h-5 w-5 ${fileChange ? "text-[var(--color-text-secondary)]" : "text-[var(--color-text-tertiary)]"}`}
                          viewBox="0 0 20 20"
                          fill="currentColor"
                        >
                          <path
                        ...
                        {fileChange && (
                          <span
                            className={`absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full ${statusDotColor(fileChange.status)}`}
                          />
                        )}
                      </div>
                    )}
