...
export function initVapid(): VapidKeys {
  ...
  if (existsSync(VAPID_PATH)) {
    try {
      ...
      if (vapidKeys) {
        webpush.setVapidDetails('https://ai.pond.audio', vapidKeys.publicKey, vapidKeys.privateKey);
...
export function addSubscription(deviceId: string, subscription: webpush.PushSubscription): void {
  const subs = loadSubscriptions();
  // Remove existing subscription for this device (replace)
  const filtered = subs.filter(s => s.deviceId !== deviceId);
  filtered.push({ deviceId, subscription, createdAt: new Date().toISOString() });
...
export async function sendPushToAll(title: string, body: string, url?: string): Promise<void> {
  ...
  for (const sub of subs) {
    try {
      ...
      if (statusCode === 410 || statusCode === 404) {
        ...
        console.log(`[push] Subscription expired for device ${sub.deviceId}, removing`);
      } else {
  ...
  if (stale.length > 0) {
    const remaining = loadSubscriptions().filter(s => !stale.includes(s.deviceId));
